...
public class Cajero implements IMetodos {
	//En el paquete com.logica se encuentra la classe de resolver las operaciones
	...
	public Ticket retirar(int numeroCuenta, double monto) {
		...
		if (consultar(numeroCuenta)!=null) {
			...
			if (monto>9000) { //validamos si el monto excede el maximo retiro cajero
				System.out.println("El monto excede el máximo permitido por el cajero");
				return ticket;
			}else if (cuenta.getSaldo()<monto) { //Validamos si el saldo es menor al monto a retirar
				System.out.println("Saldo insuficiente para realizar el retiro");
				return ticket;
			}else if((cuenta.getSaldo()-monto)<cuenta.getSaldoMin()) {
				System.out.println("El retiro dejaría por debajo del mínimo a la cuenta");
				return ticket;
			}else {//si el retiro puede actualizarse
